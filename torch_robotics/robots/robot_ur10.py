...
from torch_robotics.torch_kinematics_tree.models.robots import DifferentiableUR10
...
class RobotUr10(RobotBase):
    def __init__(self,
        ...
        self.diff_ur10 = DifferentiableUR10(
            attach_gripper=self.attach_gripper, device=tensor_args['device']
        )
        self.jl_lower, self.jl_upper, _, _ = self.diff_ur10.get_joint_limit_array()
        ...
        link_names_for_object_collision_checking = [
           # 'base_link',
            ...
            'wrist_1_link',
            'wrist_2_link',
            'wrist_3_link',  # 使用 wrist_3_link 作为碰撞检测的参考
        ]
        ...
        for link_name in link_names_for_object_collision_checking:
            idx = self.diff_ur10._name_to_idx_map[link_name]
        ...
        link_names_pairs_for_self_collision_checking = OrderedDict({
            ...
            'wrist_1_link': ['base_link','shoulder_link','upper_arm_link',],
        ...
        if use_self_collision_storm:
            ...
            self.df_collision_self = CollisionSelfFieldWrapperSTORM(
                self, 'robot_self/ur10_self_sdf.pt', self.q_dim, tensor_args=self.tensor_args)
    ...
    def fk_map_collision_impl(self, q, **kwargs):
        ...
        link_pose_dict = self.diff_ur10.compute_forward_kinematics_all_links(q, return_dict=True)
        # link_tensor = convert_link_dict_to_tensor(link_pose_dict, self.link_names_for_object_collision_checking)
        link_tensor = convert_link_dict_to_tensor(link_pose_dict, self.diff_ur10.get_link_names())
        ...
        if self.grasped_object:
            ...
            grasped_object_points_in_robot_base_frame = frame_grasped_object.transform_point(
        ...
        if grasped_object_points_in_robot_base_frame is not None:
            if len(q_orig_shape) == 3:
                grasped_object_points_in_robot_base_frame = einops.rearrange(
                    grasped_object_points_in_robot_base_frame, "(b h) d1 d2 -> b h d1 d2", b=b, h=h)
            link_pos = torch.cat((link_pos, grasped_object_points_in_robot_base_frame), dim=-2)

        return link_pos

    def get_EE_pose(self, q):
        return self.diff_ur10.compute_forward_kinematics_all_links(q, link_list=[self.link_name_ee])
    ...
    def render(self, ax, q=None, color='blue', arrow_length=0.15, arrow_alpha=1.0, arrow_linewidth=2.0,
        ...
        # draw skeleton
        skeleton = get_skeleton_from_model(self.diff_ur10, q, self.diff_ur10.get_link_names())
        skeleton.draw_skeleton(ax=ax, color=color)

        # forward kinematics
        fks_dict = self.diff_ur10.compute_forward_kinematics_all_links(q.unsqueeze(0), return_dict=True)
        ...
        if draw_links_spheres:
            ...
            link_pos = interpolate_points_v1(link_pos,
                                             self.num_interpolated_points_for_object_collision_checking).squeeze(0)
